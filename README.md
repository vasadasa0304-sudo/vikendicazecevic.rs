...
- **Guest reviews** (`shared.googleReviews` in `content.js`) are real reviews from the cabin's Google Business Profile. To refresh them: update `rating`, `total`, `captured` and `items` together, and mirror the new numbers in `llms.txt`. Rules the build enforces — quote only what a guest actually wrote, keep every written review in `items` (the "with a comment" figure is its length), and never list more quoted reviews than `total`. `textEn` is our translation and is labelled as one on `/en/`.
  - ⚠️ **Read the review text from the owner's signed-in "Read reviews" view, never from the public Maps listing.** Google machine-translates reviews on the signed-out page and does not reliably mark it — on 2026-07-27 three of the four quotes shipped in Google's reworded version, and were only caught when the owner's view showed the real wording (fixed 2026-07-28). Reproduce verbatim, including missing diacritics a guest typed.
